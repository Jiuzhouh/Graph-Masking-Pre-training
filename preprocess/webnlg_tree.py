...
def process_src(triples):
    ...
    for h, r, t in triples[:-1]:
        token = h + t
        filtered_triples.append(('S| ' + h + ' ', 'P| ' + r + ' ', 'O| ' + t + ' ', triple_dict[token] + 1))
    h, r, t = triples[-1]
    token = h + t
    filtered_triples.append(('S| ' + h + ' ', 'P| ' + r + ' ', 'O| ' + t + ' ', triple_dict[token] + 1))
    filtered_triples.sort(key=lambda x:x[-1])
...
#outdir = 'data/webnlg_pos_ordered'
outdir = 'data/webnlg20_ordered'
b = Benchmark()
#files = select_files('webnlg_challenge_2017/train')
files = select_files('webnlg_challenge_2020/train')
...
files = select_files('webnlg_challenge_2020/dev')
b.fill_benchmark(files)
...
files = [('webnlg_challenge_2020/test', 'rdf-to-text-generation-test-data-with-refs-en.xml')]
b.fill_benchmark(files)
